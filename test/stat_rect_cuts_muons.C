{
    ...
    */
    ...
    TFile *f = new TFile("/Users/musthero/Documents/Yura/Applications/TMVA-v4.2.0/test/user.yuraic.v3.muons.total.tthNTUP.root");
    TTree *tree = (TTree*)f->Get("JINRTree");

    // Declare variables of interest
    Int_t m_mu_isprompt;
    Int_t m_mu_Tight, m_mu_screwed_isprompt;
    Int_t m_mu_MuIso, m_mu_Z0SinTheta, m_mu_Sigd0PV;
    Float_t m_mu_ptcone20Dpt, m_mu_etcone20Dpt;
    ...
    for (Long64_t i=0;i<nentries;i++) {
        ...
        // Preselection cuts
        if (m_mu_isprompt==1&&m_mu_Tight==1&&m_mu_screwed_isprompt==0) { // Signal (take from TCut in TMVAMuon.C for signal):
            prompt_all++;
        } 
        else if (m_mu_isprompt==0&&m_mu_Tight==1&&m_mu_screwed_isprompt==0) { // Background (take from TCut in TMVAMuon.C for background):
            nonprompt_all++;
        } else {
            continue;
        }
        ...
        if (m_mu_Tight == 1 && m_mu_MuIso == 1 && m_mu_Z0SinTheta==1 && m_mu_Sigd0PV==1) {
            if (m_mu_isprompt==1) 
                prompt_pass++; 
            else if (m_mu_isprompt==0) 
                nonprompt_pass++;
            else
