...
filename = "/Users/musthero/Documents/Yura/Applications/TMVA-v4.2.0/test/output_muons_v2_25per.root"

profile = ROOT.TProfile("RECTCuts", "RECTCuts", 100, 0, 1, "s");

# Get TTree
file = ROOT.TFile.Open(filename)
tree = file.JINRTree2_mu2

# Declare variables of interest
m_mu_pass_Loose_Triggers = array('i', [0])
m_mu_pt = array('f',[0])
m_mu_eta = array('f',[0])
m_mu_sigd0PV = array('f',[0])
m_mu_z0SinTheta = array('f',[0])
m_mu_topoetcone20_pt = array('f',[0])
m_mu_ptvarcone20_pt = array('f',[0])
m_mu_isLoose = array('i',[0])
m_mu_truthOrigin = array('i', [0])
m_mu_isolationFixedCutTightTrackOnly = array('i', [0])

# Training

# Link variables of interest to the tree
tree.SetBranchAddress( "m_mu_pt", m_mu_pt )
tree.SetBranchAddress( "m_mu_eta", m_mu_eta )
tree.SetBranchAddress( "m_mu_sigd0PV", m_mu_sigd0PV )
tree.SetBranchAddress( "m_mu_z0SinTheta", m_mu_z0SinTheta )
tree.SetBranchAddress( "m_mu_topoetcone20_pt", m_mu_topoetcone20_pt )
tree.SetBranchAddress( "m_mu_ptvarcone20_pt", m_mu_ptvarcone20_pt )
tree.SetBranchAddress( "m_mu_isLoose", m_mu_isLoose )
tree.SetBranchAddress( "m_mu_truthOrigin", m_mu_truthOrigin )
tree.SetBranchAddress( "m_mu_pass_Loose_Triggers", m_mu_pass_Loose_Triggers )
tree.SetBranchAddress( "m_mu_isolationFixedCutTightTrackOnly", m_mu_isolationFixedCutTightTrackOnly )
...
while tree.GetEntry(c):
    ...
    if not (m_mu_isLoose[0]==1 and m_mu_pass_Loose_Triggers[0]==1): 
        continue

    isprompt = 1 if m_mu_truthOrigin[0] in (12, ) else 0
    # if m_mu_truthOrigin[0] in (12, ):    # WBoson        = 12
    #    isprompt = 1
    # elif m_mu_truthOrigin[0] in (26, ):  # BottomMeson   = 26
    #    isprompt = 0
    # else:
    #    isprompt = -1
    ...
    nonprompt_all += 1 if isprompt == 0 else 0
        ...
        # Apply standard cuts
    if m_mu_z0SinTheta[0] < 0.5 and m_mu_sigd0PV[0] < 3 and m_mu_isolationFixedCutTightTrackOnly[0] == 1:
